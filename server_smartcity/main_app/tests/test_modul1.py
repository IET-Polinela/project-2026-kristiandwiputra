...
class AuthenticationTests(APITestCase):
    ...
    def test_AUTH_03_warga_tidak_bisa_akses_halaman_admin(self):
        ...
        """
        # LANGKAH 1: Login sebagai warga biasa, bukan admin/staff
        self.client.force_login(self.warga)

        # LANGKAH 2: Tentukan URL halaman dashboard admin
        url = reverse('dashboard')

        # LANGKAH 3: Kirim GET request ke halaman dashboard
        response = self.client.get(url)

        # LANGKAH 4: Verifikasi bahwa warga biasa tidak boleh mengakses dashboard admin
        self.assertIn(
            response.status_code,
            [status.HTTP_302_FOUND, status.HTTP_403_FORBIDDEN],
            "Warga biasa tidak boleh mengakses dashboard admin."
        )