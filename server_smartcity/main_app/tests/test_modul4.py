...
class CRUDAndValidationTests(APITestCase):
    ...
    def test_FT_01_buat_laporan_dengan_data_lengkap(self):
        ...
        payload = {
            'title': 'Laporan CRUD Valid',
            'category': 'Infrastruktur',
            'description': 'Jalan berlubang dan perlu diperbaiki.',
            'location': 'Jl. Test CRUD',
            'status': 'REPORTED',
        }
        ...
        self.assertEqual(
            ...
            "POST data lengkap seharusnya menghasilkan HTTP 201 Created."
        )

        laporan = Report.objects.get(title='Laporan CRUD Valid')
    ...
    def test_FT_02_ditolak_jika_judul_kosong(self):
        ...
        payload = {
            'category': 'Infrastruktur',
            'description': 'Deskripsi ada, tetapi title tidak dikirim.',
        ...
        self.assertEqual(
            ...
            "POST tanpa title harus ditolak dengan HTTP 400 Bad Request."
        )
        self.assertIn(
            'title',
            response.data,
            "Respons validasi harus mengandung error untuk field title."
        )
    ...
    def test_FT_03_ditolak_jika_deskripsi_kosong(self):
        ...
        self.assertEqual(
            response.status_code,
            status.HTTP_400_BAD_REQUEST,
            "POST tanpa description harus ditolak dengan HTTP 400 Bad Request."
        )
        self.assertIn(
            'description',
            response.data,
            "Respons validasi harus mengandung error untuk field description."
        )
