...
from main_app.views import ReportUpdateStatusView
...
class WorkflowStateTests(APITestCase):
    ...
    def test_WF_02_tidak_bisa_edit_laporan_yang_sudah_reported(self):
        ...
        """
        # Arrange: autentikasi sebagai pemilik laporan
        self.client.force_authenticate(user=self.warga)
        ...
        # Assert: sistem menolak perubahan dan data tetap aman
        self.assertEqual(
            response.status_code,
            status.HTTP_403_FORBIDDEN,
            "Laporan REPORTED tidak boleh diubah oleh warga."
        )
    ...
    def test_WF_05_laporan_resolved_tidak_bisa_diubah(self):
        ...
        payload = {
            ...
            'description': 'Deskripsi selesai diubah paksa.',
            'location': self.laporan_resolved.location,
            'status': self.laporan_resolved.status,
        }

        # Act: warga mencoba mengubah laporan RESOLVED
        response = self.client.put(url, payload, format='json')
        ...
        self.assertEqual(
            ...
            deskripsi_awal,
            "Deskripsi laporan RESOLVED tidak boleh berubah."
        )
...
class AdminWorkflowTests(TestCase):
    ...
    def test_WF_03_admin_mengubah_status_reported_ke_verified(self):
        ...
        url = reverse('update_status', args=[self.laporan_reported.pk])

        # Act: admin mengubah status REPORTED menjadi VERIFIED
        response = self.client.post(url, {'status': 'VERIFIED'})

        # Assert: request dialihkan kembali dan status database berubah
    ...
    def test_WF_04_tidak_ada_transisi_langsung_ke_resolved_dari_reported(self):
        ...
        """
        # Arrange: ambil daftar transisi dari view update status
        allowed_transitions = ReportUpdateStatusView.allowed_transitions

        # Act: baca daftar transisi valid untuk status REPORTED
        next_statuses = allowed_transitions.get('REPORTED', [])

        # Assert: REPORTED hanya boleh menuju VERIFIED, tidak langsung RESOLVED
        self.assertIn(
            'VERIFIED',
            next_statuses,
            "Status REPORTED harus memiliki transisi lanjutan ke VERIFIED."
        )
        self.assertNotIn(
            'RESOLVED',
            next_statuses,
        ...
        self.assertEqual(
            next_statuses,
            ['VERIFIED'],
            "Transisi valid dari REPORTED hanya boleh VERIFIED."
        )